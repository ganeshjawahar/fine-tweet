...
cmd:option('-rep', '../../../auxillary/representations/word_content/doc2vec/', '')
cmd:option('-lr', 0.01, '')
	...
	while true do
		...
		if #dcontent[1] > 0 and #rcontent > 0 then
			...
			if params.word2index[neg_content] ~= nil and params.word2index[pos_content] ~= nil then
				...
				table.insert(tensors, {input, params.word2index[pos_content], 1, ug_count})
				table.insert(tensors, {input, params.word2index[neg_content], 2, ug_count})
			end
