...
cmd:option('-rep', '../../auxillary/representations/length/bow/', '')
...
params.model:add(nn.SparseLinear(50000, params.hid_size))
params.model:add(nn.ReLU())
params.model:add(nn.Linear(params.hid_size, #params.index2label))
...
function train(states)
	...
	for i = 1, #params.train_input, params.batch_size do
		...
		local cur_input, cur_target = {}, torch.CudaTensor(cur_bsize, 1)
		for j = 1, cur_bsize do
			local record = params.train_input[indices[i + j - 1]]			
			table.insert(cur_input, torch.CudaTensor(record[1]))
