...
function get_tensors(data_file, add_2_word_map)
	...
	for line in io.lines(data_file) do
		...
		if #dcontent[1] > 0 then
			...
			if params.cand2index[dcontent[2]] ~= nil then
				table.insert(tensors, {tensor, params.cand2index[dcontent[2]]})
			end
		end
...
params.model:add(nn.MaskZero(nn.Linear(2 * input_size, #params.index2cand), 1))
...
function train(states)
	...
	for i = 1, #params.train_input, params.batch_size do
		...
		local cur_input_sent, cur_input_sent_rev, cur_target = torch.CudaTensor(cur_bsize, params.max_words),torch.CudaTensor(cur_bsize, params.max_words):fill(0), torch.CudaTensor(cur_bsize, 1)
		...
		local feval = function(x)
			params.pp:copy(x)
			params.gp:zero()
			local out = params.model:forward({cur_input_sent, cur_input_sent_rev})
			...
			params.model:backward({cur_input_sent, cur_input_sent_rev}, grads)
...
function compute_performance(input)
	...
	for i = 1, #input, params.batch_size do
		...
		local cur_input_sent, cur_input_sent_rev, cur_target = torch.CudaTensor(cur_bsize, params.max_words), torch.CudaTensor(cur_bsize, params.max_words):fill(0), {}
		for j = 1, cur_bsize do
...
local dev_fscore = compute_performance(params.dev_input)
