...
cmd = torch.CmdLine()
cmd:option('-data', '../../../data/auxillary/normalization/', '')
...
params.std2index, params.index2std = tds.hash(), tds.hash()
params.nonstd2index, params.index2nonstd = tds.hash(), tds.hash()
function build_vocab(data_file)
	...
	for line in io.lines(data_file) do
		...
		if #dcontent[1] > 0 then
			...
			for j = 2, #dcontent do
				local content = stringx.split(dcontent[j], '=')
				local nonstd, std = content[1], content[2]
				if params.nonstd2index[nonstd] == nil then
					params.index2nonstd[#params.index2nonstd + 1] = nonstd
					params.nonstd2index[nonstd] = #params.index2nonstd
				end
				if params.std2index[std] == nil then
					params.index2std[#params.index2std + 1] = std
					params.std2index[std] = #params.index2std
				end
			end
		end		
	end
...
params.max_len = -1
function get_tensors(data_file, add_neg)
	local tensors = {}
	for line in io.lines(data_file) do
		local dcontent = stringx.split(line, '\t')			
		if #dcontent[1] > 0 then
			...
			for j = 2, #dcontent do
				local content = stringx.split(dcontent[j], '=')
				local nonstd, std = content[1], content[2]
				if params.nonstd2index[nonstd] ~= nil and params.std2index[std] ~= nil then
					table.insert(tensors, {tensor, params.nonstd2index[nonstd], params.std2index[std], 1, #stringx.split(dcontent[1])})
					local rand = -1
					while rand == -1 do
						local num = math.random(#params.index2std)
						if num ~= params.std2index[std] then
							rand = num
						end
					end
					table.insert(tensors, {tensor, params.nonstd2index[nonstd], rand, 2, #stringx.split(dcontent[1])})
				end
			end
...
-- params.dim = 200
params.cand1_model = nn.Sequential():add(nn.LookupTable(#params.index2nonstd, params.dim)):add(nn.Squeeze())
params.cand2_model = nn.Sequential():add(nn.LookupTable(#params.index2std, params.dim)):add(nn.Squeeze())
...
params.model:add(nn.MaskZero(nn.Linear(2 * input_size + 2 * params.dim, 2), 1))
...
function train(states)
	...
	for i = 1, #params.train_input, params.batch_size do
		...
		local cur_input_sent, cur_input_sent_rev, cur_word1, cur_word2, cur_target = torch.CudaTensor(cur_bsize, params.max_words),torch.CudaTensor(cur_bsize, params.max_words):fill(0), torch.CudaTensor(cur_bsize, 1), torch.CudaTensor(cur_bsize, 1), torch.CudaTensor(cur_bsize, 1)
		...
		local feval = function(x)
			params.pp:copy(x)
			params.gp:zero()
			local out = params.model:forward({{cur_input_sent, cur_input_sent_rev}, cur_word1, cur_word2})
			...
			params.model:backward({{cur_input_sent, cur_input_sent_rev}, cur_word1, cur_word2}, grads)
...
function compute_performance(input)
	...
	for i = 1, #input, params.batch_size do
		...
		local cur_input_sent, cur_input_sent_rev, cur_word1, cur_word2, cur_target = torch.CudaTensor(cur_bsize, params.max_words), torch.CudaTensor(cur_bsize, params.max_words):fill(0), torch.CudaTensor(cur_bsize, 1), torch.CudaTensor(cur_bsize, 1), {}
		for j = 1, cur_bsize do
