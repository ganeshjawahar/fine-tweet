...
cmd:option('-rep', '../../auxillary/representations/normalization/bow/', '')
		...
		if #dcontent[1] > 0 and #rcontent > 1 then
			...
			for j = 2, #dcontent do
				...
				if params.nonstd2index[nonstd] ~= nil and params.std2index[std] ~= nil then
					table.insert(tensors, {input, params.nonstd2index[nonstd], params.std2index[std], 1, #stringx.split(dcontent[1])})
					local rand = -1
					while rand == -1 do
					...
					table.insert(tensors, {input, params.nonstd2index[nonstd], rand, 2, #stringx.split(dcontent[1])})
				end
			end				
...
params.cand1_model = nn.Sequential():add(nn.LookupTable(#params.index2nonstd, 200)):add(nn.Squeeze())
params.cand2_model = nn.Sequential():add(nn.LookupTable(#params.index2std, 200)):add(nn.Squeeze())
...
params.model:add(nn.Linear(params.hid_size + 2*200, 2))
