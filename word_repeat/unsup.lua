...
cmd:option('-rep', '../../auxillary/representations/word_repeat/doc2vec/', '')
cmd:option('-lr', 0.01, '')
	...
	while true do
		...
		if #dcontent[1] > 0 and #rcontent > 0 then
			...
			if add_2_word_map ~= nil then
				if params.word2index[pos_tag] == nil then
					params.word2index[pos_tag] = #params.index2word + 1
					params.index2word[#params.index2word + 1] = pos_tag
				end
			...
			if params.word2index[pos_tag] ~= nil and params.word2index[neg_tag] ~= nil then
				local input = torch.Tensor(params.dim)
				for i = 1, params.dim do
					input[i] = tonumber(rcontent[i])
				end
				table.insert(tensors, {input, params.word2index[pos_tag], 1, #stringx.split(dcontent[1])})
